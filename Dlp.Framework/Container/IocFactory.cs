            ...
            if (constructorParameters.Length > 0) {
                ...
                foreach (ConstructorInfo constructorInfo in constructorInfoCollection) {
                    ...
                    foreach (Type parameterType in parameterTypeCollection) {
                        ...
                        foreach (Type constructorType in constructorParameterTypeCollection) {

                            if (constructorType == parameterType || constructorType.BaseType == parameterType) {

                                matchedParameterList.Add(constructorType);
                            }
                        }
                    }

                    //IEnumerable<Type> comparisonCollection = parameterTypeCollection.Except(constructorParameterTypeCollection);
                    //IEnumerable<Type> comparisonResultCollection = constructorParameterTypeCollection.Except(parameterTypeCollection);

                    // Caso as assinaturas dos parâmetros e do construtor sejam identicas, utiliza o construtor atual.
                    //if (comparisonCollection.Any() == false && comparisonResultCollection.Any() == false) {
