    ...
    public class DatabaseConnector : IDisposable
    {
        ...
        public void BulkInsert(string tableName, IEnumerable collection, SqlBulkCopyOptions sqlBulkCopyOptions = SqlBulkCopyOptions.Default)
        {
            ...
            foreach (PropertyInfo propertyInfo in propertyInfoCollection)
            {
                ...
                System.Attribute[] attributes = System.Attribute.GetCustomAttributes(propertyInfo);

                ColumnNameAttribute columnName = null;

                foreach (System.Attribute attribute in attributes)
                {
                    if (attribute is ColumnNameAttribute)
                    {
                        columnName = attribute as ColumnNameAttribute;
                    }
                }

                if (columnName != null && string.IsNullOrEmpty(columnName.GetName()))
                {
                    // Adiciona a nova coluna no DataTable com o nome do atributo.
                    dataTable.Columns.Add(new DataColumn(columnName.GetName(), propertyType));
                }
                else
                {
                    // Adiciona a nova coluna no DataTable com o nome da propriedade.
            ...
            do
            {
                ...
                foreach (PropertyInfo propertyInfo in propertyInfoCollection)
                {
                    dataRow[propertyInfo.Name] = propertyInfo.GetValue(enumerator.Current, null) ?? DBNull.Value;
                }
