    ...
    public class DatabaseConnector : IDisposable {
        ...
        private static IEnumerable<T> InternalReader<T>(SqlDataReader reader) {
            ...
            while (reader.Read() == true) {
                ...
                if (returnType.FullName.StartsWith("System.") == true) {
                    ...
                    IList<string> mappedProperties = new List<string>();

                    // Analisa todas as propriedades encontradas.                    
                    for (int i = 0; i < reader.FieldCount; i++) {

                        // Caso o valor da coluna seja nulo, não é necessário fazer mapeamentos adicionais.
                        if (reader.IsDBNull(i) == true) { continue; }

                        // Executa o mapeamento da propriedade encontrada.
                        ParseProperty(returnType, schemaTable, returnInstance, reader.GetName(i), reader[i], i, mappedProperties);
                    }
                }
        ...
        private static bool ParseProperty(Type returnType, DataTable schemaTable, object returnInstance, string columnName, object databaseValue, int ordinal, IList<string> mappedProperties) {
            ...
            PropertyInfo propertyInfo = returnTypeProperties.FirstOrDefault(p => p.Name.Equals(explicitClassName ?? propertyName, StringComparison.InvariantCultureIgnoreCase));

            // Extrai os dados da coluna que estamos trabalhando, a partir do schema.
            DataRow dataRow = schemaTable.Select(string.Format("ColumnName = '{0}' AND ColumnOrdinal = {1}", columnName, ordinal)).FirstOrDefault();
            ...
            if (string.IsNullOrWhiteSpace(tableName) == false) {
                ...
                if (returnType.GetProperty(tableName) != null || propertyInfo == null || mappedProperties.Contains(string.Format("{0}.{1}", returnType.Name, propertyInfo.Name)) == true) {

                    // Procura uma propriedade com o mesmo nome da tabela.
                    PropertyInfo subPropertyInfo = returnTypeProperties.FirstOrDefault(p => p.Name.Equals(tableName, StringComparison.InvariantCultureIgnoreCase));
                    ...
                    if (mappedProperties.Contains(string.Format("{0}.{1}.{2}", returnType.Name, subPropertyType.Name, propertyName)) == true) { return false; }
                    ...
                    if (subPropertyType.IsEnum == true) {
                        ...
                        mappedProperties.Add(string.Format("{0}.{1}.{2}", returnType.Name, subPropertyType.Name, propertyName));
                    ...
                    //if (ParseProperty(subPropertyType, schemaTable, subPropertyInstance, columnName, databaseValue, ordinal, mappedProperties) == true) {
                    if (ParseProperty(subPropertyType, schemaTable, subPropertyInstance, propertyName ?? columnName, databaseValue, ordinal, mappedProperties) == true) {

                        // Define o valor da propriedade encontrada.
                        subPropertyInfo.SetValue(returnInstance, subPropertyInstance, null);

                        // Adiciona a propriedade na lista de dados já mapeados.
                        mappedProperties.Add(string.Format("{0}.{1}.{2}", returnType.Name, subPropertyType.Name, subPropertyInfo.Name));
            ...
            if (propertyInfo.CanWrite == true) {
                ...
                // Adiciona a propriedade na lista de dados já mapeados.
                mappedProperties.Add(string.Format("{0}.{1}", returnType.Name, propertyName));
            }
